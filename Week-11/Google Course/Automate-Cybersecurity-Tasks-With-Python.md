...
-   **Three types:** Syntax errors, logic errors, and exceptions.

#### Syntax Errors

-   **Definition:** Invalid usage of Python language.
-   **Examples:** Forgetting to add colon after function header, omitting parenthesis after function, misspelling Python keyword, not properly closing quotation marks.
-   **Detection:** Error message indicates location of error.
-   **Fix:** Often easy to fix since you can find exactly where error happened.
-   **Comparison:** Similar to correcting simple grammar mistakes in email.

#### Logic Errors

-   **Definition:** May not cause error messages but produce unintended results.
-   **Examples:** Writing incorrect text within print statement, using wrong operator (less than instead of less than or equal to).
-   **Impact:** Can exclude values needed for code to work as intended.
-   **Example:** Reaching out to response team when priority level less than three instead of less than or equal to three.
-   **Diagnosis strategies:**
    -   **Print statements:** Insert throughout code to identify functioning sections.
    -   **Debugger:** Insert breakpoints to segment code and run portions independently.

#### Exceptions

-   **Definition:** Happen when program doesn't know how to execute code despite no syntax problems.
-   **Causes:** Mathematically impossible operations (divide by 0), accessing non-existent index values, unrecognized variable/function names, incorrect data types.
-   **Example:** Accessing index 100 in string "security" (which has 8 characters) causes "string index out of range" error.
-   **Diagnosis:** Can use debuggers and print statements to figure out potential source of error.

# Certificate Completed

![Certificate proof in case I have to pay for it](Screenshot%202025-07-31%20at%2010.46.10 PM.png)
